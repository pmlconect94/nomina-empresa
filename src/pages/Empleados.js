...
export default function Empleados({ userRol }) {
  ...
  function iniciarNuevo() {
    setEditando('nuevo')
    setForm({ id_banco:'', id_toka:'', nombre:'', area:'', puesto:'', sd_fiscal:0, sd_real:0, infonavit:0, activo:true })
    setNuevo(true)
  }
  ...
  async function guardar() {
    if (nuevo) {
      await supabase.from('empleados').insert({
        id_banco: +form.id_banco, id_toka: +form.id_toka,
        nombre: form.nombre, area: form.area, puesto: form.puesto,
        sd_fiscal: +form.sd_fiscal, sd_real: +form.sd_real,
        infonavit: +form.infonavit, activo: true
      })
    } else {
      await supabase.from('empleados').update({
        nombre: form.nombre, area: form.area, puesto: form.puesto,
        sd_fiscal: +form.sd_fiscal, sd_real: +form.sd_real,
        infonavit: +form.infonavit, activo: form.activo
      }).eq('id', editando)
    }
    setEditando(null)
    fetchEmpleados()
  ...
  return (
    <div className="empleados-page">
          ...
          <div className="modal-card">
            ...
            <div className="modal-body">
              <div className="form-grid">
                ...
                <div className="form-field full"><label>Nombre completo</label><input type="text" value={form.nombre||''} onChange={e=>f('nombre',e.target.value)}/></div>
      ...
      <table className="emp-table">
        <thead>
          <tr>
            <th>ID Banco</th><th>Nombre</th><th>Área</th><th>Puesto</th>
            <th>Sueldo fiscal</th><th>Sueldo real</th><th>Infonavit</th><th>Status</th>
        ...
        <tbody>
          {empleados.map(e=>(
            <tr key={e.id} className={!e.activo?'inactivo':''}>
              ...
              <td><span className={`status-badge ${e.activo?'activo':'inactivo'}`}>{e.activo?'Activo':'Inactivo'}</span></td>
              {canEdit && (
