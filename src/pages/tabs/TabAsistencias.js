...
export default function TabAsistencias({ semana, nominas, empleados, asistencias, canEdit }) {
  ...
  async function updateAsist(nomId, diaIdx, fecha, campo, valor) {
    ...
    const updated = {
      ...(existente || { nomina_id: nomId, dia_index: diaIdx, fecha, codigo: '', te_horas: 0, te_motivo: '', retardo_min: 0 }),
      [campo]: valor
    }
    setLocalAsist(prev => ({ ...prev, [key]: updated }))

    // Bugfix: Sincroniza silenciosamente el arreglo padre para que al cambiar de pestaña no se pierda la data
    if (!asistencias[nomId]) asistencias[nomId] = []
    const idxInProp = asistencias[nomId].findIndex(a => a.dia_index === diaIdx)
    if (idxInProp >= 0) asistencias[nomId][idxInProp] = updated
    else asistencias[nomId].push(updated)
    ...
    try {
      if (existente?.id) {
        ...
        if (nueva) {
          ...
          const idxProp = asistencias[nomId]?.findIndex(a => a.dia_index === diaIdx)
          if (idxProp >= 0) asistencias[nomId][idxProp].id = nueva.id
        }
      }
    } catch (err) {
  ...
  function handleKeyDown(e, empIdx, diaIdx) {
    if (e.key === 'Tab') {
      e.preventDefault();
      const nextEmpIdx = e.shiftKey ? empIdx - 1 : empIdx + 1;
      
      // Buscar el siguiente select si existe dentro de los límites
      if (nextEmpIdx >= 0 && nextEmpIdx < empleados.length) {
        document.getElementById(`codigo-${nextEmpIdx}-${diaIdx}`)?.focus();
      }
    }
  }
                      ...
                      <React.Fragment key={i}>
                        <td className="td-asist bl" style={{ background: bgColor }}>
                          <select
                            ...
                            onKeyDown={e => handleKeyDown(e, empIdx, i)}
                          >
                            <option value="">—</option>
