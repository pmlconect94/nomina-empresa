      ...
      {prestamos.length === 0 ? (
        ...
        <div className="tabla-card">
          <table>
            ...
            <tbody>
              {prestamos.map(p => {
                // Bug #2: calcular descuento por préstamo individual, no el total del empleado
                const descPorPrestamo = Math.min(
                  p.tipo === 'semanal' ? p.monto * 0.10 : p.monto * 0.20,
                  p.saldo
                )
                const saldoDespues = Math.max(0, p.saldo - descPorPrestamo)
                return (
                  <tr key={p.id}>
                    <td>
                    ...
                    <td style={{textAlign:'right'}} className="red bold">-{fmt(descPorPrestamo)}</td>
