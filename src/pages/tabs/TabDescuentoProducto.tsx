...
export function TabDescuentoProducto({ semana, nominas, empleados, canEdit, onChanged }: any) {
  ...
  const [form, setForm] = useState<any>({ empleado_id: '', monto: '', numero_nota: '' });
  const [saving, setSaving] = useState(false);

  useEffect(() => { fetchItems(); }, [semana.id]);
  async function fetchItems() {
    const { data } = await supabase.from('nomina_descuento_producto')
  ...
  async function agregar() {
    ...
    const { error } = await supabase.from('nomina_descuento_producto').insert({
      semana_id: semana.id, nomina_id: nomId, empleado_id: form.empleado_id, monto, numero_nota: form.numero_nota.trim() || null,
    });
    if (error) { toast.error(error.message); setSaving(false); return; }
    toast.success('Descuento agregado');
    setForm({ empleado_id: '', monto: '', numero_nota: '' }); setSaving(false); fetchItems(); onChanged?.();
  }
  ...
  return (
    <div>
      ...
      {canEdit && (
        <div className="card" style={{ marginBottom: 14 }}><div className="card-body">
          <div className="form-grid form-grid-3">
            ...
            <div><label className="field-label">Monto a descontar</label><input className="field-input mono" type="number" step="0.01" value={form.monto} onChange={(e) => setForm((f: any) => ({ ...f, monto: e.target.value }))} /></div>
          ...
          <div className="hstack" style={{ justifyContent: 'flex-end', marginTop: 12 }}>
            <button className="btn btn-primary btn-sm" onClick={agregar} disabled={saving}><Icon name="plus" size={14} /> Agregar descuento</button>
          </div>
        </div></div>
      )}
      ...
      <div className="card tbl-wrap">
        <table className="tbl">
          <thead><tr><th>Empleado</th><th>N° nota</th><th className="right">Monto</th><th>Fecha</th>{canEdit && <th></th>}</tr></thead>
          <tbody>
            {items.length === 0 && <tr><td colSpan={canEdit ? 5 : 4}><div className="empty"><div className="empty-title">Sin descuentos de producto</div></div></td></tr>}
            {items.map((i) => (
              <tr key={i.id}>
