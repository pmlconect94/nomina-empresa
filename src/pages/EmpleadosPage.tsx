...
export function EmpleadosPage() {
  ...
  function abrirEdicion(e: any) { setForm({ ...e }); setNuevo(false); setEditando(e.id); }
  ...
  async function toggleImss(e: any) {
    if (!canEdit) return;
    const nuevoVal = !e.alta_imss;
    setEmpleados((prev) => prev.map((x) => x.id === e.id ? { ...x, alta_imss: nuevoVal } : x));
    const { error } = await supabase.from('empleados').update({ alta_imss: nuevoVal }).eq('id', e.id);
              ...
              <tr key={e.id} className={!e.activo ? 'row-inactive' : ''}>
                ...
                <td className="center">
                  <div className="hstack" style={{ gap: 8, justifyContent: 'center' }}>
                    <button className={`switch ${e.alta_imss ? 'on' : ''}`} onClick={() => toggleImss(e)} disabled={!canEdit} title={e.alta_imss ? 'Alta: transferencia + vales' : 'Sin alta: todo en efectivo'} />
        ...
        <div className="modal-backdrop" onClick={(e) => e.target === e.currentTarget && setEditando(null)}>
          <div className="modal page-enter" style={{ maxWidth: 1180, width: '96vw' }}>
            ...
            <div className="modal-body">
              ...
              <div className="form-section-title">Esquema de pago / IMSS</div>
              <div className="hstack" style={{ gap: 12, background: 'var(--ink-50)', borderRadius: 'var(--r-md)', padding: '12px 14px', border: '1px solid var(--ink-200)' }}>
                <button type="button" className={`switch ${form.alta_imss ? 'on' : ''}`} onClick={() => f('alta_imss', !form.alta_imss)} />
                <div>
                  <div className="fw-600">{form.alta_imss ? 'Dado de alta en IMSS' : 'Sin alta en IMSS'}</div>
                  <div className="text-xs muted">{form.alta_imss ? 'Se le paga por transferencia bancaria + vales de despensa.' : 'Se le paga todo en efectivo.'}</div>
                </div>
              </div>
              ...
              <p className="text-xs muted" style={{ marginTop: 18 }}>El sueldo, infonavit y su historial se gestionan en la pantalla protegida <strong>SUELDO</strong> (fase F2).</p>
            </div>
