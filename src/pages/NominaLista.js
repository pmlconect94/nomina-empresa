...
export default function NominaLista({ userRol, onAbrirNomina }) {
  ...
  const [filtro, setFiltro] = useState('abierta') // #2 default abiertas
  ...
  async function crearNomina() {
    ...
    const { data: semana, error } = await supabase.from('semanas').insert({
      fecha_inicio: fechaIni, fecha_fin: fechaFin, tipo, status: 'abierta'
    }).select().single()
    if (!error) {
  ...
  // #1 eliminar nómina abierta
  async function eliminarNomina(e, semana) {
    e.stopPropagation()
    if (!window.confirm(`¿Eliminar la nómina "${fmtPeriodo(semana.fecha_inicio, semana.fecha_fin)}"?\nEsta acción no se puede deshacer.`)) return
    await supabase.from('semanas').delete().eq('id', semana.id)
    fetchSemanas()
  }
      ...
      <div className="filtros">
        {['todas','abierta','timbrada'].map(f => (
          <button key={f} className={`filtro ${filtro===f?'active':''}`} onClick={() => setFiltro(f)}>
            {f === 'todas' ? 'Todas' : f === 'abierta' ? 'Abiertas' : 'Guardadas'}
          </button>
        ))}
      </div>
        ...
        {lista.map(s => (
          <div key={s.id} className="nomina-card" onClick={() => onAbrirNomina(s)}>
            ...
            <div className="nc-right">
              <span className={`badge-status ${s.status}`}>{s.status === 'abierta' ? 'Abierta' : 'Guardada'}</span>
              {canEdit && s.status === 'abierta' && (
                <button className="btn-del-nomina" onClick={e => eliminarNomina(e, s)} title="Eliminar nómina">✕</button>
              )}
              ...
              {tipo && (
                <>
                  ...
                  <div className="form-grid-2">
                    <div className="form-field"><label>Fecha inicio</label><input type="date" value={fechaIni} onChange={e => setFechaIni(e.target.value)} /></div>
                    <div className="form-field"><label>Fecha fin</label><input type="date" value={fechaFin} onChange={e => setFechaFin(e.target.value)} /></div>
                  </div>
                  {fechaIni && fechaFin && (
