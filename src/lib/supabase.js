...
export function calcularNomina(empleado, nomina, asistencias, incentivosViaje, descuentoPrestamo) {
  ...
  const totalRetHrs = dias.reduce((s,d) => s + (d.retardo_min||0)/60, 0)
  ...
  const infonavit = nomina?.infonavit || empleado.infonavit || 0
  const totalDed  = infonavit
    + (nomina?.comedor || 0)
    + retardoMonto
    + (nomina?.prestamos || 0)
    + (descuentoPrestamo || 0)
...
export function descuentoPrestamo(monto, tipo) {
